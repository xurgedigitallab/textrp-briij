...
"""This is a mypy plugin for Briij to deal with some of the funky typing that
...
PREFER_SYNAPSE_CLOCK_CALL_LATER = ErrorCode(
    ...
    "Prefer using `textrp_briij.util.Clock.call_later` instead of `reactor.callLater`",
    category="synapse-reactor-clock",
)

PREFER_SYNAPSE_CLOCK_LOOPING_CALL = ErrorCode(
    "prefer-synapse-clock-looping-call",
    "Prefer using `textrp_briij.util.Clock.looping_call` instead of `task.LoopingCall`",
    category="synapse-reactor-clock",
)

PREFER_SYNAPSE_CLOCK_CALL_WHEN_RUNNING = ErrorCode(
    "prefer-synapse-clock-call-when-running",
    "Prefer using `textrp_briij.util.Clock.call_when_running` instead of `reactor.callWhenRunning`",
    category="synapse-reactor-clock",
)

PREFER_SYNAPSE_CLOCK_ADD_SYSTEM_EVENT_TRIGGER = ErrorCode(
    "prefer-synapse-clock-add-system-event-trigger",
    "Prefer using `textrp_briij.util.Clock.add_system_event_trigger` instead of `reactor.addSystemEventTrigger`",
    category="synapse-reactor-clock",
)
...
prometheus_metric_fullname_to_label_arg_map: Mapping[str, ArgLocation | None] = {
    ...
    "textrp_briij.metrics.LaterGauge": ArgLocation("labelnames", 2),
    "textrp_briij.metrics._InFlightGaugeRuntime": ArgLocation("labels", 2),
    "textrp_briij.metrics.InFlightGauge": ArgLocation("labels", 2),
    "textrp_briij.metrics.GaugeBucketCollector": ArgLocation("labelnames", 2),
    ...
    "textrp_briij.metrics._gc.GCCounts": None,
    "textrp_briij.metrics._gc.PyPyGCStats": None,
    "textrp_briij.metrics._reactor_metrics.ReactorLastSeenMetric": None,
    "textrp_briij.metrics.CPUMetrics": None,
    "textrp_briij.metrics.jemalloc.JemallocCollector": None,
    "textrp_briij.util.metrics.DynamicCollectorRegistry": None,
    "textrp_briij.metrics.background_process_metrics._Collector": None,
    #
    # `Metric` subclasses:
    ...
    "textrp_briij.metrics.GaugeHistogramMetricFamilyWithLabels": ArgLocation(
        "labelnames", 4
    ),
}
"""
...
(`textrp_briij.metrics._types.Collector`) or `prometheus_client.metrics_core.Metric`. The
...
class BriijPlugin(Plugin):
    ...
    def get_function_signature_hook(
        ...
        # functions. ex. `textrp_briij.metrics.jemalloc.JemallocCollector@185` (this is the line
        # number)
        if "@" in fullname:
        ...
        if fullname == "textrp_briij.util.clock.Clock":
            return check_clock_creation

        if (
            fullname
            == "textrp_briij.metrics.background_process_metrics.run_as_background_process"
        ):
    ...
    def get_method_signature_hook(
        ...
        if fullname.startswith(
            (
                "textrp_briij.util.caches.descriptors.CachedFunction.__call__",
                "textrp_briij.util.caches.descriptors._LruCachedFunction.__call__",
            )
        ):
            return cached_function_method_signature

        if fullname in (
            "textrp_briij.util.caches.descriptors._CachedFunctionDescriptor.__call__",
            "textrp_briij.util.caches.descriptors._CachedListFunctionDescriptor.__call__",
        ):
            return check_is_cacheable_wrapper

        if fullname in (
            "twisted.internet.interfaces.IReactorTime.callLater",
            "textrp_briij.types.ISynapseThreadlessReactor.callLater",
            "textrp_briij.types.IBriijReactor.callLater",
        ):
            return check_call_later

        if fullname in (
            "twisted.internet.interfaces.IReactorCore.callWhenRunning",
            "textrp_briij.types.ISynapseThreadlessReactor.callWhenRunning",
            "textrp_briij.types.IBriijReactor.callWhenRunning",
        ):
            return check_call_when_running

        if fullname in (
            "twisted.internet.interfaces.IReactorCore.addSystemEventTrigger",
            "textrp_briij.types.ISynapseThreadlessReactor.addSystemEventTrigger",
            "textrp_briij.types.IBriijReactor.addSystemEventTrigger",
        ):
...
def check_call_later(ctx: MethodSigContext) -> CallableType:
    ...
    `textrp_briij.util.Clock.call_later` should always be used instead of `reactor.callLater`.
    This is because the `textrp_briij.util.Clock` tracks delayed calls in order to cancel any
    ...
    using `textrp_briij.util.Clock.call_later` with `call_later_cancel_on_shutdown` set to
    ...
    signature: CallableType = ctx.default_signature
    ctx.api.fail(
        "Expected all `reactor.callLater` calls to use `textrp_briij.util.Clock.call_later` "
...
def check_looping_call(ctx: FunctionSigContext) -> CallableType:
    ...
    `textrp_briij.util.Clock.looping_call` should always be used instead of `task.LoopingCall`.
    `textrp_briij.util.Clock` tracks looping calls in order to cancel any outstanding calls
    during server shutdown.
    ...
    signature: CallableType = ctx.default_signature
    ctx.api.fail(
        "Expected all `task.LoopingCall` instances to use `textrp_briij.util.Clock.looping_call` "
        "instead. This is so that long lived calls can be tracked for cancellation during "
        "server shutdown",
        ctx.context,
...
def check_call_when_running(ctx: MethodSigContext) -> CallableType:
    ...
    `textrp_briij.util.Clock.call_when_running` should always be used instead of
    `reactor.callWhenRunning`.

    Since `reactor.callWhenRunning` is a reactor callback, the callback will start out
    with the sentinel logcontext. `textrp_briij.util.Clock` starts a default logcontext as we
    ...
    ctx.api.fail(
        (
            "Expected all `reactor.callWhenRunning` calls to use `textrp_briij.util.Clock.call_when_running` instead. "
            "This is so all Briij code runs with a logcontext as we want to know which server the logs came from."
        ),
        ctx.context,
...
def check_add_system_event_trigger(ctx: MethodSigContext) -> CallableType:
    ...
    `textrp_briij.util.Clock.add_system_event_trigger` should always be used instead of
    ...
    ctx.api.fail(
        (
            "Expected all `reactor.addSystemEventTrigger` calls to use `textrp_briij.util.Clock.add_system_event_trigger` instead. "
            "This is so all Briij code runs with a logcontext as we want to know which server the logs came from."
        ),
        ctx.context,
...
def analyze_prometheus_metric_classes(ctx: ClassDefContext) -> None:
    ...
    if any(
        ...
        in (
            ...
            "textrp_briij.metrics._types.Collector",
...
def check_prometheus_metric_instantiation(
    ...
    This is important because we support multiple Briij instances running in the same
    ...
    if isinstance(labelnames_arg_expression, (ListExpr, TupleExpr)):
        ...
        for labelname_expression in labelnames_arg_expression.items:
            if (
                ...
                and labelname_expression.fullname == "textrp_briij.metrics.SERVER_NAME_LABEL"
            ):
...
# Types defined in Briij which are known to be immutable.
IMMUTABLE_CUSTOM_TYPES = {
    "textrp_briij.synapse_rust.acl.ServerAclEvaluator",
    "textrp_briij.synapse_rust.push.FilteredPushRules",
    # This is technically not immutable, but close enough.
    "signedjson.types.VerifyKey",
    "textrp_briij.types.StrCollection",
}
...
def plugin(version: str) -> type[BriijPlugin]:
    ...
    # string.
    #
    # However, since we pin the version of mypy Briij uses in CI, we don't
    # really care.
