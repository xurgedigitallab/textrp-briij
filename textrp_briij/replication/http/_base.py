...
from textrp_briij.api.errors import HttpResponseException, SynapseError
from textrp_briij.config.workers import MAIN_PROCESS_INSTANCE_NAME
from textrp_briij.http import RequestTimedOutError
...
from textrp_briij.logging.opentracing import trace_with_opname
...
from textrp_briij.util.cancellation import is_function_cancellable
...
class ReplicationEndpoint(metaclass=abc.ABCMeta):
    ...
    This creates an endpoint under `/_briij/replication/:NAME/:PATH_ARGS..`
    ...
    with `CACHE` set to true then this generates an endpoint:
        ...
        /_briij/replication/send_event/:event_id/:txn_id
        ...
        async def send_request(
            *, instance_name: str = MAIN_PROCESS_INSTANCE_NAME, **kwargs: Any
            ...
            with outgoing_gauge.track_inprogress():
                ...
                uri = "briij-replication://%s/_briij/replication/%s/%s" % (
                    instance_name,
                    cls.NAME,
                    "/".join(url_args),
                )
    ...
    def register(self, http_server: HttpServer) -> None:
        ...
        pattern = re.compile("^/_briij/replication/%s/%s$" % (self.NAME, args))
