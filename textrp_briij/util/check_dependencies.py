...
"""
This module exposes a single function which checks textrp_briij's dependencies are present
...
DISTRIBUTION_NAME = "textrp-briij"
...
def _not_installed(requirement: Requirement, extra: str | None = None) -> str:
    if extra:
        return (
            f"Briij by TextRP {VERSION} needs {requirement.name} for {extra}, "
            f"but it is not installed"
        )
    else:
        return f"Briij by TextRP {VERSION} needs {requirement.name}, but it is not installed"
...
def _incorrect_version(
    ...
    if extra:
        return (
            f"Briij by TextRP {VERSION} needs {requirement} for {extra}, "
            f"but got {requirement.name}=={got}"
        )
    else:
        return (
            f"Briij by TextRP {VERSION} needs {requirement}, but got {requirement.name}=={got}"
        )
...
def _no_reported_version(requirement: Requirement, extra: str | None = None) -> str:
    if extra:
        ...
        return (
            f"Briij by TextRP {VERSION} needs {requirement}, "
            f"but can't determine {requirement.name}'s version"
        )
...
def check_requirements(extra: str | None = None) -> None:
    ...
    `pip install textrp-briij[saml2]`).
    ...
    if extra is None:
        ...
        raise ValueError(
            f"Briij by TextRP {VERSION} does not provide the feature '{extra}'"
        )
