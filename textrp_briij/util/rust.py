...
import textrp_briij
from textrp_briij.synapse_rust import get_rust_file_digest
...
def check_rust_lib_up_to_date() -> None:
    ...
    briij_root = get_briij_source_directory()
    if briij_root is None:
        return None

    # Get the hash of all Rust source files
    rust_path = os.path.join(briij_root, "rust")
    if not os.path.exists(rust_path):
        return None

    hash = _hash_rust_files_in_directory(briij_root)

    if hash != get_rust_file_digest():
        raise Exception("Rust module outdated. Please rebuild using `poetry install`")


def _hash_rust_files_in_directory(briij_root: str) -> str:
    """Get the hash of all files in a directory (recursively)"""

    src_directory = os.path.abspath(os.path.join(briij_root, "rust", "src"))
    ...
    paths.append(os.path.join(briij_root, "rust", "Cargo.toml"))
    paths.append(os.path.join(briij_root, "rust", "build.rs"))
    paths.append(os.path.join(briij_root, "Cargo.lock"))
    paths.append(os.path.join(briij_root, "Cargo.toml"))
...
def get_briij_source_directory() -> str | None:
    """Try and find the source directory of briij for editable installs (like
    ...
    # Try and find the installed textrp-briij package.
    try:
        package = Distribution.from_name("textrp-briij")
        ...
        synapse_dir = os.path.dirname(textrp_briij.__file__)
    ...
    if direct_url_json is None:
        ...
        #
        # An egg-info install is when there exists a `textrp_briij.egg-info`
        ...
        # `textrp_briij.egg-info/PKG-INFO` file, and going up two directories
        # from there.

        metadata_path = package.locate_file("textrp_briij.egg-info/PKG-INFO")
