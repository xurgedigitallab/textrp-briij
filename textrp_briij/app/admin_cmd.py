...
from textrp_briij.handlers.admin import ExfiltrationWriter
from textrp_briij.server import HomeServer
from textrp_briij.storage.database import DatabasePool, LoggingDatabaseConnection
...
from textrp_briij.types import IBriijReactor, JsonMapping, StateMap
from textrp_briij.util.logcontext import LoggingContext

logger = logging.getLogger("textrp_briij.app.admin_cmd")
...
def create_homeserver(
    ...
    if config.worker.worker_app is not None:
        assert config.worker.worker_app == "textrp_briij.app.admin_cmd"

    # Update the config with some basic overrides so that don't have to specify
    # a full worker config.
    config.worker.worker_app = "textrp_briij.app.admin_cmd"
