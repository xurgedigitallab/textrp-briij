...
import textrp_briij.util.caches
from textrp_briij.api.constants import MAX_REQUEST_SIZE
from textrp_briij.app import check_bind_error
from textrp_briij.config import ConfigError
from textrp_briij.config._base import format_config_error
from textrp_briij.config.homeserver import HomeServerConfig
from textrp_briij.config.server import (
    ListenerConfig,
    ManholeConfig,
    TCPListenerConfig,
    UnixListenerConfig,
...
from textrp_briij.http.site import SynapseSite
from textrp_briij.logging.context import LoggingContext, PreserveLoggingContext
from textrp_briij.metrics import install_gc_manager, register_threadpool
from textrp_briij.metrics.jemalloc import setup_jemalloc_stats
...
from textrp_briij.types import IBriijReactor, StrCollection
from textrp_briij.util import SYNAPSE_VERSION
from textrp_briij.util.caches.lrucache import setup_expire_lru_cache_entries
from textrp_briij.util.daemonize import daemonize_process
from textrp_briij.util.gai_resolver import GAIResolver
from textrp_briij.util.rlimit import change_resource_limit
...
def start_worker_reactor(
    ...
    # (needed when textrp_briij.app.complement_fork_starter is being used)
...
def listen_metrics(
    ...
    from textrp_briij.metrics import RegistryProxy
...
def listen_manhole(
    ...
    from textrp_briij.util.manhole import manhole
...
def listen_http(
    ...
    site = SynapseSite(
        logger_name="textrp_briij.access.%s.%s"
...
def reload_cache_config(config: HomeServerConfig) -> None:
    ...
    Note that the following cache config keys are read, but not applied:
        ...
        - track_memory_usage. This affects textrp_briij.util.caches.TRACK_MEMORY_USAGE which
...
def setup_sentry(hs: "HomeServer") -> None:
    ...
    app = (
        ...
        else "textrp_briij.app.homeserver"
    )
