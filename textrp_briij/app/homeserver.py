...
from textrp_briij.config._base import ConfigError, format_config_error
...
from textrp_briij.util.module_loader import load_module

logger = logging.getLogger("textrp_briij.app.homeserver")
...
class BriijHomeServer(HomeServer):
    """
    Homeserver class for the main Briij process.
    """
    ...
    def _listener_http(
        ...
        # 2. Redirect to the static "Briij is running" page.
    ...
    def _configure_named_resource(
            ...
            resources.update(
                {
                    ...
                    **build_briij_client_resource_tree(self),
                }
            )

            if self.config.email.can_verify_email:
                from textrp_briij.rest.briij.client.password_reset import (
                    PasswordResetSubmitTokenResource,
                )

                resources["/_briij/client/password_reset/email/submit_token"] = (
                    PasswordResetSubmitTokenResource(self)
                )

        if name == "consent":
            from textrp_briij.rest.consent.consent_resource import ConsentResource
        ...
        if name in ["static", "client"]:
            resources[STATIC_PREFIX] = StaticResource(
                os.path.join(os.path.dirname(textrp_briij.__file__), "static")
            )
...
def create_homeserver(
    ...
    """
    Create a homeserver instance for the Briij main process.
    ...
    if config.worker.worker_app:
        raise ConfigError(
            ...
            "instance. Please use `python -m textrp_briij.app.generic_worker` instead (or remove the option if this is the main process)."
        )
...
def setup(
    ...
    """
    Setup a `BriijHomeServer` (main) instance.
