...
from textrp_briij.http import QuieterFileBodyProducer, RequestTimedOutError, redact_uri
from textrp_briij.http.proxyagent import ProxyAgent
from textrp_briij.http.replicationagent import ReplicationAgent
...
from textrp_briij.logging.opentracing import set_tag, start_active_span, tags
from textrp_briij.metrics import SERVER_NAME_LABEL
from textrp_briij.types import IBriijReactor, StrSequence
from textrp_briij.util.async_helpers import timeout_deferred
...
# IBriijReactor implies IReactorCore, but explicitly marking it this as an implementer
# of IReactorCore seems to keep mypy-zope happier.
@implementer(IReactorCore, IBriijReactor)
...
class SimpleHttpClient(BaseHttpClient):
    ...
    def __init__(
        ...
        if self._ip_blocklist:
            ...
            self.reactor: IBriijReactor = BlocklistingReactorWrapper(
...
def is_unknown_endpoint(
    ...
    if synapse_error is None:
        synapse_error = e.to_briij_error()
