...
from textrp_briij.config.server import generate_ip_set, parse_proxy_config
from textrp_briij.types import JsonDict
from textrp_briij.util.check_dependencies import check_requirements
...
class MediaUploadLimit:
    ...
    These can be configured through the `media_upload_limits` [config option](https://docs.briij.example/synapse/latest/usage/configuration/config_documentation.html#media_upload_limits)
    or via the `get_media_upload_limits_for_user` module API [callback](https://docs.briij.example/synapse/latest/modules/media_repository_callbacks.html#get_media_upload_limits_for_user).
    """
...
class ContentRepositoryConfig(Config):
    ...
    def read_config(self, config: JsonDict, **kwargs: Any) -> None:
        ...
        if (
            ...
            and config.get("worker_app") != "textrp_briij.app.media_repository"
        ):
        ...
        for i, provider_config in enumerate(storage_providers):
            ...
            if (
                ...
                or provider_config["module"] == "textrp_briij.rest.media.v1.storage_provider"
            ):
                provider_config["module"] = (
                    "textrp_briij.media.storage_provider.FileStorageProviderBackend"
                )
