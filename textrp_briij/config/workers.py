...
from textrp_briij.config._base import (
    Config,
    ConfigError,
...
from textrp_briij.config._util import parse_and_validate_mapping
...
from textrp_briij.util.pydantic_models import ParseModel
...
documentation here:
`https://docs.briij.example/synapse/latest/usage/configuration/config_documentation.html#instance_map`
"""
...
`https://docs.briij.example/synapse/latest/workers.html#worker-configuration`
"""
...
class WorkerConfig(Config):
    ...
    def read_config(
        ...
        if self.worker_app == "textrp_briij.app.homeserver":
        ...
        # The shared secret used for authentication when connecting to the main textrp_briij.
        ...
        self.should_notify_appservices = self._should_this_worker_perform_duty(
            ...
            legacy_worker_app_name="textrp_briij.app.appservice",
        ...
        self.should_update_user_directory = self._should_this_worker_perform_duty(
            ...
            legacy_worker_app_name="textrp_briij.app.user_dir",
    ...
    def _should_this_worker_perform_duty(
        ...
        Parameters:
            ...
            legacy_worker_app_name: The name of a legacy Synapse worker application
                ...
                e.g. "textrp_briij.app.appservice"
        ...
        if self.worker_app == legacy_worker_app_name and config.get(
            ...
            raise ConfigError(
                ...
                f"Consider setting `worker_app: textrp_briij.app.generic_worker` and using the '{new_option_name}' option instead.\n"
        ...
        if (
            ...
            if new_option_should_run_here != legacy_option_should_run_here:
                update_worker_type = (
                    " and set worker_app: textrp_briij.app.generic_worker"
    ...
    def _worker_names_performing_this_duty(
        ...
        1. The old way where "start_pushers" is set to false and running a
          `textrp_briij.app.pusher'` worker app.
        ...
        Args:
            ...
            legacy_app_name: The historical app name. e.g. 'textrp_briij.app.pusher'
