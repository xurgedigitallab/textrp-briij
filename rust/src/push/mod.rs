...
pub fn register_module(py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    // textrp_briij.synapse_rust import push` work (and keep legacy aliases).
    let modules = py.import("sys")?.getattr("modules")?;
    modules.set_item("textrp_briij.synapse_rust.push", &child_module)?;
    modules.set_item("synapse.synapse_rust.push", &child_module)?;
