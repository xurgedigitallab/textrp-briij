...
# Configuring Briij by TextRP

This is intended as a guide to the Briij by TextRP configuration. The behavior of a Briij by TextRP instance can be modified
...
For example, setting `max_avatar_size: 10M` means that Briij by TextRP will not accept files larger than 10,485,760 bytes
for a user avatar.
