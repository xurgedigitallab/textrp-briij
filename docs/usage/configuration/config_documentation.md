...
Example configuration:
```yaml
web_client_location: https://briij.example/
```
---
### `public_baseurl`
...
* `app_name` (string): Defines the default value for `%(app)s` in `notif_from` and email subjects. Defaults to `"Briij by TextRP"`.
...
email:
  ...
  app_name: my_branded_briij_server
  enable_notifs: true
  notif_for_new_users: false
  client_base_url: http://localhost/briij
...
If you're using the [example systemd service](https://github.com/textrp/briij-synapse/blob/develop/contrib/systemd/textrp-briij.service) file in Briij by TextRP's `contrib` directory, you can send a `SIGHUP` signal by using `systemctl reload textrp-briij`.
...
sso:
  client_whitelist:
  - https://app.briij.example/develop
...
* `enabled` (boolean): Set to `true` to enable native XRPL/Xahau wallet authentication. Defaults to `false`.

* `allow_account_creation` (boolean): Whether an unknown wallet address may create a new local Matrix account after a successful signed challenge response. Defaults to `true`.

* `xrpl_node_url` (string): JSON-RPC endpoint for the XRPL network. Defaults to `"https://s.altnet.rippletest.net:51234"`.

* `xahau_node_url` (string): JSON-RPC endpoint for the Xahau network. Defaults to `"https://xahau-testnet.xrpl-labs.com"`.

* `challenge_ttl_seconds` (integer): Number of seconds that an XRPL login challenge remains valid before it expires and must be requested again. Defaults to `300`.
