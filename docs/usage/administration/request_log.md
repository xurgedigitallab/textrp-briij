...
HTTP request logs are written by synapse (see [`synapse/http/site.py`](https://github.com/textrp/briij-synapse/tree/develop/synapse/http/site.py) for details).
...
| GGGG  | Server Port |
| HHHH  | Federated Server or Local User making request (blank if unauthenticated or not supplied).<br/>If this is of the form `@aaa:example.com|@bbb:example.com`, then that means that `@aaa:example.com` is authenticated but they are controlling `@bbb:example.com`, e.g. if `aaa` is controlling `bbb` [via the admin API](https://docs.briij.example/synapse/latest/admin_api/user_admin_api.html#login-as-a-user). |
